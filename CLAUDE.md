...
Page sequence: `startPage` → `infoPage` → `consentPage` → `page0` (task instructions) → `sampleGuidePage` (guide plus two worked examples) → question pages → `finalPage`. Static pages navigate via `showStaticPage(id)`.
...
Test emails bypass the counter. Use `?assignment=0` through `?assignment=29` to inspect any schedule. The four attention checks required by `../ethics.pdf` are implemented. The Sample Page Guide contains two criterion-specific worked examples with answers shown directly; their assets live in `training_examples/`. These examples do not implement the answer-and-retry mandatory training described in `../ethics.pdf`.
